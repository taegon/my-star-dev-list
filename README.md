...
* 박해선 님: "핸즈온 머신러닝", "케라스 창시자에게 배우는 딥러닝", "파이썬 라이브러리를 활용한 머신러닝" 등의 역자. 엄청난 양의 책의 번역을 소화하고 있다. 직접 저술하거나 번역한 책이 너무 많아서 로드맵을 봐야할 수준.
...
* 류광 님: 기술서적 번역자로서 아주 유명한 역자. 내 관심사와는 다소 거리가 있어서 책을 많이 읽어보진 못했지만, 도서관이나 서점 가면 언제나 볼 수 있던 이름이었다.
  * [류광의 번역 이야기](http://occamsrazr.net/tt/)
...
* 엄태웅 (Terry Taewoong Um) 님: 페이스북에서 찾은 분. 캐나다에서 대학원 생활을 하는데, 열정과 실력이 있다. 머신러닝 관련해서 자료를 찾다보면, 이름을 자주 발견할 수 있다. 데이터 지능 팟캐스트에도 출연했었다. 우리가 3didas.net를 통해 지향했던 것과 같은 "대학원생 때 알았더라면 좋았을 것들"이라는 블로그를 정리하였고, 결국 책으로 출판까지 하였다. 👍 김태훈님이 천재스타개발자 같은 느낌이라면, 엄태웅님은 이상적인 대학원생의 모습이랄까. 학교 다닐때 옆에 두고 많이 배우고 싶은 친구. 현재는 학업도 잠깐 접어두고 사업을 하고 계시다. 아트랩의 창업자, CEO이다.
...
* 박찬성 님: 케라스 코리아 오픈책과 TensorFlow KR 페이스북 그룹에서 알게 되었는데, 2020년 한해 왕성하게 활동하더니, 책도 쓰셨다. fast.ai KR 페이스북 커뮤니티를 운영하고 있기도 하다. ETRI 소속. 활발한 활동으로 GDE for Machine Learning로 활동하시고 계시다. 최근 시계열 분석과 머신러닝 포켓레퍼런스 등을 번역하셨다.
  * [![GitHub][6.2] deep-diver](https://github.com/deep-diver)
  * "나만의 스마트워크 환경 만들기 : 파이썬으로 만드는 업무 자동화 프로그램"의 저자

## 프론트엔드/백엔드

* xguru(권정혁) 님: 꽤 오래전부터 블로그를 열심히 찾아봤던 개발자이다. 최근에는 거의 찾아보지 못했다. 트위터를 팔로우하고 있는데, 왜 안 보였는지 모르겠다. 글쓰기 앱을 10일전쯤 공개한 것이 있다. 레진코믹스와 우아한 형제 등을 거치셨다. 내가 동경하던 개발자로 손에 꼽던 분. 요즘 가장 자주 들어가는 GeekNews의 운영자이시기도 하다.
...
* 송성광(saltfactory)님: 하이브레인넷 개발자이다. 학교에 적을 두고, 개발을 해간다는 게 쉽지 않은데, 학습하고 학습한 내용을 기록으로 남기는 점이 대단하였다. 이 분 블로그도 매일 들락거리면서 봤던 기억이 있다. 이 분 글을 보고, github pages로 홈페이지를 만들 생각을 했던 것으로 기억한다. 성장하는 모습을 더 볼 수 있으면 좋겠는데, 최근에는 생업이 바쁘셔서인지, 모습을 보기 어려워 안타까운 마음이 있다.
...
* 김태곤(taggon) 님: 나와 동명이인인 자바스크립트 개발자. 네이버, 팬시를 거쳐 지금은 Automattic에서 유일한 한국인 개발자로 일하고 계시다. `.kim`도메인이 열렸을 때, 나도 살까 고민했던 도메인을 이분에게 뺐겻다(??). 소속회사 방침상 책을 쓰지 못한다고 하신다. 블로그에 가끔 글을 올리시는데, 역시나 좋은 글이어서 읽는 즐거움이 있다.
...
* 이동욱님: EO채널에서 처음 얼굴을 뵈었던 개발자분인데, 유투브를 운영하고 계시고 굉장히 유명한 개발자이다. 취업 준비하면서 찾아보다가 유투브 영상을 몇개 찾아보았는데, 열심히 노력하시고, 노력에 비례해서 성장하고 계신 모습이 멋있다. 최근에는 오랫동안 몸담았던 배민을 떠나, 인프런으로 옮기셨다. 백엔드 개발자. 자바 스프링을 주로 쓰신다.
  * [이동욱님 개인 블로그](https://jojoldu.github.io/)

## 아직 분류 안함

* 윤석찬 님: 임백준 저자님 만큼이나 유명하고, 오래동안 봐왔던 분. 다음, 서울대 등에서 일할 때부터 꾸준히 블로그를 써오셨고, 현재는 AWS에서 에반젤리스트로 일하고 계시다. 여전히 활발히 블로그를 작성하고 계시고, "차니의 클라우드 클리닉"이라는 팟캐스트도 운영하고 계시다. AWS에서 일해보고 싶다는 생각이 들게끔 한 분.
